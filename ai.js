...
const { saveStack, stackTitles, stackTop, stackPop } = require('./tasks')
const { c, color } = require('./lib/colors')
const { sendChat, debugChat, logEvent } = require('./utils')
const { createProvider } = require('./ai-provider')
const { logChatDB, logTaskAction } = require('./memory')
  ...
  async function streamAndProcess(msgs) {
    ...
    if (pendingActions.length > 0) {
      ...
      if (stopIdx !== -1) {
        ...
        require('./engine').interrupt()  // clears queue + sets abortSignal
      }
